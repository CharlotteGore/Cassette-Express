...
var Manifest = base.createChild().addInstanceMethods({
	...
	createAssembly : function( key ){
		...
		var convertToSortable = function( key ){
			...
			var single = self.assetIndex[key].fileDependencies();
		...
		_.each(initialFiles, function(file){
			...
			filesThatNeedFiles = filesThatNeedFiles.concat(convertToSortable(file));

		});

		// process 2: take files we have and generate a list of files we need. 
		var filesNeeded = [];
		var finishedList = [];

		_.each(filesThatNeedFiles, function(file, index){

			if(!_.isEmpty(file.dependsOn)){
				// oops, it has some depencencies... 

				_.each(file.dependsOn, function( dependency ){

					if(!_.find(filesWeHave, function(item){ return num.key === file.key; }){
				...
				// file has no dependencies. Move from filesThatNeedFiles to finishedList.
				// We no longer care about it. 
				finishedList.push( filesWeHave.slice(index, 1) );
		...
		/*
			...
			while files we don't have > 0
				...
				'get' the file, i.e, create a 'sortable' version of the file.
				put it into files we have
